...
public class Controlador implements PersistenciaBatallas {
    ...
    public Controlador(boolean esGui) {
        ...
        if (!archivoBatallas.exists() || archivoBatallas.length() == 0) {
            // Si el archivo no existe o está vacío, inicializamos la persistencia con una
            // lista vacía
    ...
    // Este método asegura que solo haya una instancia del controlador en toda la
    // aplicación vista.mostrarLogro
        ...
        switch (escena) {
            ...
            case 3:
                if (vista.isError() == false) {
                    entrenador1 = Entrenador.capturarEntrenador(listaEntrenadores.get(0), listaPokemones.get(0),
                            listaPokemones.get(1), listaPokemones.get(2));
            ...
            case 5:
                if (vista.isError() == false) {
                    entrenador2 = Entrenador.capturarEntrenador(listaEntrenadores.get(1), listaPokemones.get(0),
                            listaPokemones.get(1), listaPokemones.get(2));
    ...
    public void atacar(Ataque ataqueElegido) {
        // Entonces cada vez que seleccione un ataque tiene que llamar a este método y
        // pasarle el ataque
    ...
    public void ordenarContrincantes() {
        // Este método será el que usaremos para avanzar de escena entre la elección de
        // pokemones y la pelea
    ...
    public boolean isFinDeBatalla() {
        if (escena == 7) {
            return true; // La batalla ha terminado
        }
        if (batalla == null) {
            boolean entrenador1SinPokemons = batalla.getEntrenador1().getEquipo().stream()
                    .allMatch(p -> p.getHp() <= 0);
            boolean entrenador2SinPokemons = batalla.getEntrenador2().getEquipo().stream()
                    .allMatch(p -> p.getHp() <= 0);
            return entrenador1SinPokemons || entrenador2SinPokemons;
        }
        return false;
